...
キャラクター候補：
- **Reinhard** — 完璧主義な設計批評。「なぜその設計でなければならないか」を詰める。extends機構・install先の設計に向く
- **Saul** — 現実的なリスク・抜け穴を探す弁護士視点。update戦略の穴（上書き問題・破壊的変更）に向く
- **Tonegawa** — 実務者視点で「本当に使えるか」を問う。kit中身の設計・AskOS固有 vs 汎用の分離に向く
- **House** — 斜め上からの診断。「そもそも前提が間違っていないか」を突く。全体の設計前提の検証に向く
- **Yang** — 楽観的な推進役。批判に対するカウンターと可能性の提示。バランス取りに1人入れると良い
