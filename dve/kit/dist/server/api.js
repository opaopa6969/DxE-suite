...
export function startAPIServer(config, port = 4174) {
...
`;
        ...
        if (req.method === "GET" && url.pathname === "/api/drift") {
            ...
            for (const proj of config.projectDirs) {
                ...
                if (!existsSync(gp))
                    continue;
                const graph = JSON.parse(readFileSync(gp, "utf-8"));
                const ddNodes = graph.nodes.filter((n) => n.type === "decision");
                for (const dd of ddNodes) {
                    const data = dd.data;
                    if (!data.file_path || data.status === "overturned")
                        continue;
                    // Check git: files changed since DD date
                    try {
                        const since = data.date || "2020-01-01";
                        const log = execSync(`git log --oneline --since="${since}" -- .`, { cwd: proj.path, encoding: "utf-8", timeout: 5000 }).trim();
                        if (log.split("\n").length > 5) {
        ...
        if (req.method === "GET" && url.pathname === "/api/coverage") {
            ...
            for (const session of sessions) {
                ...
                for (const char of chars) {
                    if (!charGaps[char])
                        charGaps[char] = { total: 0, critical: 0, high: 0 };
                    charGaps[char].total += sessionGaps.length;
                    charGaps[char].critical += sessionGaps.filter((g) => g.data.severity === "Critical").length;
                    charGaps[char].high += sessionGaps.filter((g) => g.data.severity === "High").length;
                }
            }
        ...
        if (req.method === "POST" && url.pathname === "/api/scan") {
            ...
            function scanRec(dir, depth) {
                ...
                if (isProject) {
                    ...
                    if (existsSync(sd))
                        try {
                            sessions = readdirSync(sd).filter((f) => f.endsWith(".md") && f !== "index.md").length;
                        }
                        catch { }
                    if (existsSync(dd))
                        try {
                            decisions = readdirSync(dd).filter((f) => f.endsWith(".md") && f !== "index.md").length;
                        }
                        catch { }
        ...
        if (req.method === "GET" && url.pathname === "/api/status") {
            const states = config.projectDirs.map((p) => detectProjectState(p.name, p.path));
            return json(res, {
                projects: states,
                stateChart: {
