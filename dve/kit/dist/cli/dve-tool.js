                ...
                for (const [patterns, toolkit] of CAPABILITIES) {
                    for (const pat of patterns) {
                        try {
                            ...
                            const found = ex(`find "${r.path}" -path "*/node_modules" -prune -o -path "*/.git" -prune -o -path "*/dge" -prune -o -path "*/dre" -prune -o -path "*/dve" -prune -o -path "*/dde" -prune -o -name "*${pat}*" -print 2>/dev/null | head -3`, { encoding: "utf-8", timeout: 5000 }).trim();
                            if (found) {
                                if (projFindings === 0)
                                    console.log(`  ${r.name}:`);
