...
- **シチュエーション変化**: 場面転換（時間・場所・状況を変える）、圧力状況（締め切り・障害）、傍観者の導入
- **キャラクター深化**: バックストーリー開示（過去の経験を対話に織り込む）、弱点の発動（weakness が実際に表出する）、立場の変化（Session 内で賛否が変わる）
- **対話技法**: サブテキスト（言葉の裏の本音）、コールバック（序盤の伏線を終盤で回収）、沈黙・間（意味のある間）
- **メタ技法**: 劇中劇（キャラがユーザーを演じる）、第四の壁を破る（読者に問いかける）、異化効果（「当たり前」を問い直す）

### 演出技法と Gap の関係
演出技法は「面白さ」のためだけに使わない。各技法は特定の種類の Gap を露出させる道具である:
- 場面転換 → 環境依存の Gap
- バックストーリー → 過去の失敗パターン
- サブテキスト → 表面的合意の裏の未解決リスク
- 圧力状況 → 障害時の手順・ロールバックの Gap
- 劇中劇 → UX ウォークスルーの欠落

### 弱点発動ルール
キャラの `weakness` は 1 Session に最低 1 回発動させること。weakness が発動した結果、他のキャラが修正・補完する流れが Gap 発見の質を高める。

### 立場変化ルール
最低 1 キャラは Session 内で立場（賛成/反対）が変わること。変化の瞬間が Gap 発見のトリガーになる。
