...
| # | タスク | 出典 | 種類 |
|---|--------|------|------|
| 22 | **キャラの「沈黙・ボディランゲージ」表現を許可する** — 応答義務ルールを緩和し、発言なしでもボディランゲージ（「*上目づかいで見渡す*」「*腕を組んで黙る*」等）で存在を表現できるようにする。「沈黙は言葉」。今泉のような観察型キャラに特に有効 | 会話 | スキル改善 |
| 23 | **キャラのブレ防止** — axis（評価軸）に沿わない発言をしていないか判定する仕組み。今泉が専門家的発言をするなど役割逸脱が起きやすい | 会話 | スキル改善 |
